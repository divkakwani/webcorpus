...
class TransliterationScheme(dict):
    ...
    def __init__(self, blockName, schemeName, data, swapTable=None):
        ...
        if swapTable is not None:
            if not isinstance(swapTable, dict):
                raise TypeError
            ...
            if colIndex < len(self._parsedata[rowIndex]):
                ...
                if c != prevchar:
                    ...
                    if prevchar is not None:
                        self._setupParseTree(
                            rowFrom, rowIndex - 1, colIndex + 1, tree[prevchar]
                        )
                    rowFrom = rowIndex
                    prevchar = c
                if rowIndex == rowTo:
                    self._setupParseTree(
                        rowFrom, rowIndex, colIndex + 1, tree[prevchar]
                    )
            rowIndex = rowIndex + 1
    ...
    def __getNextChar(self, text, startPos, tree):
        ...
        if i < len(text) - 1:
            try:
                ...
                # Ignore. The lookup for the equivalent character later on
                # will handle it.
                pass
        return matched
...
def transliterate(text, inFormat, outFormat, requestOptions={}):
    ...
    try:
        ...
        if inBlock is not outBlock:
            raise ValueError("incompatible input and output formats")

        if inFormat is outFormat:
            # They're trying to trick us. Just do a quick sanity check & bounce
            # it back.
...
class DevanagariCharacter(TLCharacter):
    ...
    """
        ...
        Unicode Data doesn't help with this - category 'Mn' is not unique
        to dependent vowels - so need to hard code the ranges
    """
    ...
    def __init__(self, unicodeHexValue, block):
        ...
        if unicodeHexValue == 0x960:
            # dependency vowel sign for vocalic RR is set only when
            # processing the vowel, since the maatra precedes the vowel
            # in the Unicode chart
        ...
        if unicodeHexValue in DevanagariCharacter._depVowelRange:
            ...
            if unicodeHexValue == 0x962:
                ...
                # dependency vowel sign for vocalic RR is set only when
                # processing the vowel, since the maatra precedes the
                # vowel in the Unicode chart
                # That step's cpde is above, with documentation
                pass
            else:
                vowel = block[chr(unicodeHexValue - DevanagariCharacter._vowelOffset)]
            if vowel is not None:
                # The check condition is for 0x944, processing deferred for
                # later
        ...
        if (
            self.isVowel is False
            and self.chr.isalpha()
            and self.unicodeHexValue
            not in (DevanagariCharacter._AGRAVAHA, DevanagariCharacter._OM)
        ):
...
class DevanagariCharacterBlock(CharacterBlock, _Devanagari):
    ...
    def _equivalent(self, char, prev, next, implicitA):
        ...
        """
        result = []
        if char.isVowel is False:
            result.append(char.chr)
            if char.isConsonant and (
                (next is not None and next.isConsonant) or next is None
            ):
                result.append(DevanagariCharacter._VIRAMA)
        else:
            if prev is None or prev.isConsonant is False:
                result.append(char.chr)
            else:
...
class DevanagariTransliterationScheme(TransliterationScheme, _Devanagari):
    ...
    def __init__(self, blockName, schemeName, data, swapTable=None):
        ...
        """
        TransliterationScheme.__init__(self, blockName, schemeName, data, swapTable)
    ...
    def _equivalent(self, char, prev, next, implicitA):
        ...
        if (
            ...
            result.append(
                characterBlocks["DEVANAGARI"][
                    DevanagariCharacter._LETTER_A
                ].equivalents[self.name]
            )
        return result
...
def main(argv=None):
    ...
    text -- the text to be transliterated OR the name of a file containing the
            text
    inputFormat -- the name of the character block or transliteration scheme
                   that the text is to be transliterated FROM, e.g. 'CYRILLIC',
                   'IAST'. Not case-sensitive
    outputFormat -- the name of the character block or transliteration scheme
                    that the text is to be transliterated TO, e.g. 'CYRILLIC',
                    'IAST'. Not case-sensitive
    """
    if argv is None:
        argv = sys.argv
